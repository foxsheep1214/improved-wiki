...
class CreatePageReasonNamesTheFile(unittest.TestCase):
    """NashSU builds this from created[].fileName (review-view.tsx:250).

    A reason carrying the human title leaves an audit trail that cannot locate
    the page it claims to have created.
    """

    _NOW = datetime(2026, 8, 11, 14, 30, 52, tzinfo=timezone.utc)

    def test_single_page_reason_is_an_openable_path(self):
        d = ra.route_review_action(
            _item(title="Gate Charge Losses"), "Create Page",
            has_search_source=True, now=self._NOW)
        self.assertEqual(
            d["resolve_reason"],
            "Created: wiki/queries/gate-charge-losses-2026-08-11-143052.md",
        )

    def test_every_draft_carries_the_filename_to_write(self):
        d = ra.route_review_action(
            _item(review_type="missing-page",
                  title="Missing pages: Alpha, Beta"),
            "Create Page", has_search_source=True, now=self._NOW)
        self.assertEqual(
            [draft["file_name"] for draft in d["drafts"]],
            ["alpha-2026-08-11-143052.md", "beta-2026-08-11-143052.md"],
        )

    def test_multi_page_reason_counts_instead_of_listing(self):
        d = ra.route_review_action(
            _item(review_type="missing-page",
                  title="Missing pages: Alpha, Beta"),
            "Create Page", has_search_source=True, now=self._NOW)
        self.assertEqual(d["resolve_reason"], "Created 2 pages")

    def test_filename_scheme_is_shared_with_the_research_writer(self):
        # One naming rule for every non-ingest page, as NashSU routes all of
        # them through wiki-filename.ts.
        import _wiki_filename
        import write_research_page

        self.assertIs(
            write_research_page.make_query_slug, _wiki_filename.make_query_slug)
