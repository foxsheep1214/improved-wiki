...
class TestStructuralGate(unittest.TestCase):
    ...
    def test_malformed_yaml_frontmatter_is_rejected(self):
        malformed = _page("body").replace(
            "title: Book", 'title: "9.2 "Forward" and "Flyback""')
        with self.assertRaisesRegex(RuntimeError, "valid YAML frontmatter"):
            source_page._validate_source_file_block(malformed, "book")

    def test_frontmatter_must_be_a_mapping(self):
        non_mapping = _page("body").replace(
            "type: source\ntitle: Book\ntags: []\nrelated: []\n"
            'sources: ["raw/book.pdf"]',
            "- source\n- raw/book.pdf",
        )
        with self.assertRaisesRegex(RuntimeError, "YAML mapping"):
            source_page._validate_source_file_block(non_mapping, "book")
