"""Tests for _stage_3_2_stamp_frontmatter_dates — frontmatter round-trip cleanliness.

Regression: _stage_3_2_stamp_frontmatter_dates used ``content[3:end]`` to slice the
...
class TestStampFrontmatterDates(unittest.TestCase):
    def test_no_blank_line_after_opening_fence(self):
        out = s3._stage_3_2_stamp_frontmatter_dates(FM, "2026-06-20")
        self.assertTrue(out.startswith("---\ntype: source"),
                        f"expected no blank line after ---; got: {out[:30]!r}")

    def test_idempotent_no_stacked_blank_lines(self):
        once = s3._stage_3_2_stamp_frontmatter_dates(FM, "2026-06-20")
        twice = s3._stage_3_2_stamp_frontmatter_dates(once, "2026-06-20")
    ...
    def test_dates_stamped(self):
        out = s3._stage_3_2_stamp_frontmatter_dates(
            "---\ntype: source\ntitle: X\n---\nbody", "2026-06-20")
