...
_GOOD_SYNTHESIS = (
    "## Result\n"
    "The measured gate charge dominates switching loss in this topology, and "
    "the reported figures line up across both vendor datasheets and the "
    "independent bench measurements collected here [1]."
)


class TestResearchSynthesisValidation(unittest.TestCase):
    """NashSU 0.6.8 keeps incomplete synthesis retryable instead of saving it."""

    def test_accepts_substantive_cited_synthesis(self):
        result = writer.validate_research_synthesis(_GOOD_SYNTHESIS, 1)
        self.assertTrue(result.valid)
        self.assertIsNone(result.error)
        self.assertEqual(result.cited_source_indexes, [1])

    def test_rejects_empty_and_too_short_synthesis(self):
        for content in ("", "   \n\n  ", "Too short [1]."):
            with self.subTest(content=content):
                result = writer.validate_research_synthesis(content, 1)
                self.assertFalse(result.valid)
                self.assertIn("empty or incomplete", result.error)

    def test_rejects_reasoning_only_output(self):
        # A stream can succeed and still carry no assistant prose. Without a
        # gate this becomes a references-only artifact.
        content = (
            "<think>Internal planning that is easily long enough on its own to "
            "clear any raw length threshold applied before cleanup.</think>"
        )
        result = writer.validate_research_synthesis(content, 1)
        self.assertFalse(result.valid)
    ...
    def test_rejects_uncited_synthesis_when_sources_exist(self):
        uncited = _GOOD_SYNTHESIS.replace(" [1]", "")
        result = writer.validate_research_synthesis(uncited, 1)
        self.assertFalse(result.valid)
        self.assertIn("did not cite", result.error)

    def test_uncited_is_allowed_when_there_are_no_sources(self):
        uncited = _GOOD_SYNTHESIS.replace(" [1]", "")
        self.assertTrue(writer.validate_research_synthesis(uncited, 0).valid)

    def test_citation_markers_parse_lists_ranges_and_ignore_out_of_range(self):
        self.assertEqual(
            writer.cited_research_source_indexes("a [1] b [3,2] c", 3),
            [1, 2, 3],
        )
        self.assertEqual(
            writer.cited_research_source_indexes("span [2-4]", 5),
            [2, 3, 4],
        )
        self.assertEqual(writer.cited_research_source_indexes("[9]", 3), [])
        self.assertEqual(writer.cited_research_source_indexes("[0]", 3), [])
        self.assertEqual(writer.cited_research_source_indexes("[-]", 3), [])
...
class TestWriterCli(unittest.TestCase):
    ...
    def test_incomplete_synthesis_writes_nothing_and_stays_retryable(self):
        for label, synthesis in (
            ("reasoning-only", "<think>planning only, no answer body</think>"),
            ("uncited", _GOOD_SYNTHESIS.replace(" [1]", "")),
        ):
            with self.subTest(label=label):
                with tempfile.TemporaryDirectory() as raw_tmp:
                    ...
                    self.assertIn("retry", stderr.getvalue().lower())
                    self.assertFalse((root / "wiki" / "queries").exists())
