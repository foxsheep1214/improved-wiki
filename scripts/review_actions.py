...
def route_review_action(item: dict, action: str, *,
    ...
    if _looks_like_research(action):
        if not has_search_source:
            return create_page_decision(item, action)
    ...
    if action.startswith(_CREATE_PAGE_SENTINEL):
        return create_page_decision(item, action[len(_CREATE_PAGE_SENTINEL):])
...
def create_page_decision(item: dict, action: str) -> dict:
    drafts = create_review_page_drafts(item, action)
    reason = (f"Created: wiki/{drafts[0]['dir']}/{drafts[0]['title']}"
