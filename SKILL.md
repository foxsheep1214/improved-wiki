...
- **Batch ingest**: `python3 scripts/ingest.py f1.pdf f2.pdf ...` — pass all books in **one command**; the pipeline auto-parallelizes the wiki-independent prefetch (Phase 0/1 + Stage 2.2) across books while the wiki-dependent spine (Stage 2.3→write) runs one book at a time. **Do not manually split books into separate `ingest.py` calls**, and never run two books' 2.3+ in parallel — only the spine is serial. Concurrency caps (`--parallel` sub-agent limit), the in-book Stage 2.4 parallel exception (2026-07-09), and prefetch mechanics: `references/batch-parallel-prefetch.md` (authoritative).
...
- **Semantic lint batching**: `wiki-lint-semantic.py` splits page summaries into context-derived char-budget batches (2026-07-10; replaces the old fixed 200-page split — reuses ingest's `_compute_chunk_targets` formula with its own 256K-token hard ceiling, `IMPROVED_WIKI_LINT_TARGET_TOKENS_CEIL` env override) so it scales to large wikis without blowing context; batches are disjoint by page and only deduped once at the end, so a single invocation eager-drains and emits prompts for ALL currently-uncached batches at once (parallel-eager, same pattern as ingest.py Stage 2.4) rather than one round-trip at a time — dispatch one subagent per emitted prompt
