...
3. **Stage 3.1/3.5 (review)**: Generate, validate, then persist review items
4. **Stage 3.3**: Update index/log/overview
...
The review item's `search_queries` field (generated by Stage 3.1 for
`suggestion`/`missing-page` reviews) provides 2–3 keyword-rich web search
queries that seed Step 2b directly. Its `affected_pages` field identifies the
wiki context to read. If an older review lacks `search_queries`, derive them
from `title` and `affected_pages`.
...
Both auto-ingest and deep research use the same Stage 3.2 write path, Stage 3.3
aggregate repair, and cache lifecycle. The wiki does not need a separate write
protocol for research-derived knowledge.
