...
`source_reclassification` 按账本追加顺序回放来源位置，不把所有旧路径建立成
永久跳转图。支持 A→B→A 和随后再次分类；旧路径被一次新的 ingest 复用时，
新记录形成独立历史链。移动从已腾空且未复用的路径发起，或目标已有相同 hash
的独立历史链时，作为冲突拒绝，避免合并来源身份。
追加接口先验证候选账本能完整投影，再落盘；既有事件的原始字节不重写。
查询返回当前路径，并以 `source_at_event` / `source_page_at_event` 保留原始位置。
