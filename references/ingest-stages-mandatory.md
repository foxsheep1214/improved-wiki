---
name: improved-wiki
description: "improved-wiki 活跃 Ingest Stage + Lint + Graph 规范，每 Stage 含作用/产物/go-no-go。"
...
improved-wiki 流水线跨 Phase 0–3；编号即执行顺序。Stage 2.2 滚动产生
document digest，Stage 2.4 统一生成 source、concept/entity 与所有
schema-typed 页。Lint 与 Graph 是独立命令，不属于 ingest 管线。
...
| 3.1 | `stage_3_1_prepare_review_suggestions` | 写盘前对 in-memory FILE generation 运行内容质量审查并严格校验；只保存 checkpoint，不写 REVIEW 页 |
| 3.2 | `stage_3_2_write_wiki_file` | 文件写盘（含同名 slug 三层 page-merge，NashSU parity） |
| 3.3 | `stage_3_3_aggregate_repair` | 聚合修复（index/log/overview） |
| 3.4 | `stage_3_4_inject_images` | 图片注入 source 页 |
| 3.5 | `stage_3_5_persist_review_suggestions` | 将 3.1 已校验结果写入 REVIEW 页与 runtime JSON |
...
`validate_ingest.py` 是独立手动工具，不是 ingest stage。恢复兼容保留的
`stage_2_9_done` marker 现仅代表 Stage 2.4 去重收尾与源页保证。
...
- **流程**（2026-07-08 起代码强制）：`_do_prepare` 对每个候选文件调 `stage_0_1_check_file`——schema.md 缺失或无规则块 → raise（先起草规则）；违规 → raise（`normalize_raw_names.py --fix` 重命名后重跑）。范围与全库扫描一致：仅检查规则声明文件夹下的 `.pdf`（`.md` 天然放行，含用户另行明确触发的 `wiki/queries/*.md` 兼容 ingest；v0.6.7 Deep Research 默认不触发）；warn 级启发式不阻断。全库批量检查/修复仍用 `normalize_raw_names.py --check/--fix`。
- **go/no-go**：候选文件全部合规。
...
- **NashSU 对齐**：`accumulated_digest` 初始为空，每 chunk 产出 `updated_global_digest` 滚动合并。2.2 完成后，最终 `accumulated_digest` 解析回 dict 作 `global_digest` 给 Stage 2.4。短源（1 chunk）= 整本 digest。`updated_global_digest` 必含 5 字段（book_meta/outline/key_entities/key_concepts/key_claims），可选第六字段 `schema_typed_candidates` 只保留后续 chunk 需要的真正重要候选；首 chunk 建立 book_meta+outline。
...
- **per-handoff subagent 隔离**：每个 chunk 交给一个 fresh subagent；主对话只编排。见 `delegate-mode.md`。
- **existing-slugs 相关性 cap**：chunk prompt 里的已有 wiki 页按"slug token 在本 chunk 文本中的包含率"确定性排序，取前 `_EXISTING_SLUGS_CAP=1000`（≈40K 字符，对齐 NashSU index trim；Stage 2.4 也有同类 cap），保持 prompt 哈希跨 resume 稳定。
...
### Stage 3.5 · Review Persistence
- **作用**：把 Stage 3.1 已验证的同一批 items 持久化为 `wiki/REVIEW/<type>/<date>-<source>-<slug>.md` 与 `review-suggestions.json`，不做第二次 LLM 调用。
- **go/no-go**：只消费 `review_prepared`；成功后以 `review_done` 绑定 review page refs。
...
关键依赖：
- 1.2 先于 1.3（先有图才能 caption）；1.2/1.3 先于 3.4（注入图引用）
- 2.2 对所有源运行（短源 1 chunk / 长源 N chunk）；2.2 必须全部 chunk 分析完才进 2.3
- 2.3 在 2.2 与 2.4 之间检测已存在 wiki 关联（wiki 为空跳过）；2.4 对整书只生成一次，同一次调用产出强制源页，随后收尾跑源内去重（单 chunk 跳过）
- Phase 2 全在内存（2.3→2.4 串行），产出统一由 3.2 写盘
...
| Stage | 门禁检查 |
|-------|---------|
| 2.2 | chunk 分析结果齐全且无 error；滚动汇总 digest 含 5 必需 key 且类型正确（无 ≥1 concept 数量门槛） |
