...
| 2.9 | `stage_2_9_comparison_generation` | 源内对比生成（2.9A 跨域消歧义已删，见上方说明） |
...
- **作用**：源内概念对比（两个高度相关概念 → 对比页，对比维度 ≥4，至多 2 页）。详见 `comparison-generation.md`。原 2.9A 跨域消歧义页生成已删（2026-06-26 NashSU 对齐：同名 slug 由 3.1 page-merge 接住，不另建 hub 页）。
...
### Stage 3.3 · ~~Cross-domain Slug Collision Review~~（已删 2026-06-26）
- **已移除**：NashSU 无 `domain` 字段、无消歧义概念。同名 slug 在 3.1 写盘时走三层 page-merge 接住，不再事后审查/警告。编号 3.3 保留为占位以避免重排后续编号，但无代码、无 go/no-go。
...
- **3.1 写盘时同名 slug 走 page-merge**（NashSU parity，2026-06-26 起；原 3.3 碰撞审查已删）
