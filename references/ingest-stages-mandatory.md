...
| 4.1 | `stage_4_1_validate_ingest`（`_stage_4_1_validate.py`）/ `validate_ingest.py` | 最终验证 |
...
- **作用**：与 2.2 合并为 barrier-free pipeline：analyze chunk → generate pages → next chunk。每 chunk 分析完立即生成概念/实体页。仅生成 source/concept/entity 三种 page type。
...
- **子步骤（生成后）· 源页生成**：所有 chunk 生成完，`stage_2_6_source_page` 从 global digest 生成源页（本书索引，列出概念/实体/问题/对比），并入 file_blocks。与 NashSU Step 2 把 source page 作为生成产物 item 1 对齐。go/no-go：source page 路径为 `wiki/sources/<stem>.md`。
...
- **作用**：ingest 末尾自动运行 `validate_ingest.py`（13 阶段全验证），结果打印到 stdout。遵循 Iron Law：NO COMPLETION CLAIMS WITHOUT FRESH VERIFICATION EVIDENCE。
...
```
0.1 → 0.2 → 1.1 → 1.2 → 1.3 → 2.1 → 2.2 → 2.4 → 2.5 → 2.7 → 2.8 → 2.9
...
- 2.4 内含增量关联验证子步骤（wiki 为空跳过）；2.5 依赖 2.4（单 chunk 跳过）
- Phase 2 全在内存（2.4→2.5→2.7→2.8→2.9 串行），产出统一由 3.1 写盘
...
Graph 不在 ingest 管线内。Ingest 管线不碰图——图建在 Graph 命令，图用在 Ingest 之外（`--mode query --slug <page>` 是只读工具，为任意页面返回 top-N 建议缺失 wikilink，不自动改文件、不在 ingest 管线内调用）。触发：完成一批 ingest 后手动运行，或 `AUTO_BUILD_GRAPH=1` 自动触发（30 分钟新鲜度守卫）。详见 `graph.py --help`。
