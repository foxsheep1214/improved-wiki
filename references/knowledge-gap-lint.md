...
> **触发**：每次 `wiki-lint` 运行时自动扫描，或在 ingest 完成后由 Stage 2.6 联动触发。
...
synthesis / finding / thesis / methodology 的形成不取决于单次 ingest 消化了什么——它们取决于**整个 wiki 的累积状态**。因此放在 per-ingest Stage 2.5 里跑是不对的：99% 的时候白算，而且语义不属于"本次 ingest 的产物质量审查"。
...
| claims cross-source | Stage 1.5 chunk analysis `claims` + concept 页面内容 | LLM 判断指向同一结论 |
...
- **2026-06-17**：初版。从 Stage 2.5 拆分而来——高层知识空缺检测的语义属于 lint（wiki 整体健康）而非 per-ingest review（本次产物质量）。
