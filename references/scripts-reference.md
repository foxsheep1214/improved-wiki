...
| Ingest orchestrator splits | `ingest.py` (CLI + `ingest_one`/`batch_ingest`) → `_batch_coordination.py` (coordinator flock + durable spine reservation + prefetch pause), `_batch_worker_status.py` (worker heartbeat + lease), `_ingest_skip.py` (Stage 0.2 去重/skip), `_ingest_chunks.py` (chunk 流水线 + bounded Stage 2.4 waves), `_ingest_prepare.py` (综合/source page), `_ingest_write.py` (写盘 + post-ingest) |
