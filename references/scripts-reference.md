...
| Lint | **`wiki-lint.sh`**（默认结构+语义+emit-review+fix+fix-links+sweep+一轮 dedup；随后 exit 102 等用户确认 delete-orphans；确认后用 `--delete-orphans-only`；`--diagnostic-only`/`--structural-only` 为非修改模式）, `wiki-lint-fix.py`（修复/孤页删除执行器）, `wiki-lint-semantic.py`, `lint_verify_semantic.py`（improved-wiki 独有：对 severity=="warning" 语义发现用全文再核验一遍，非 NashSU parity，lint 后手动跑），**`validate_ingest.py`**, `normalize_raw_names.py` |
| Graph | **`graph.py`** |
