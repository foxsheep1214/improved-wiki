...
| Repair | `sweep_reviews.py`, `batch_resolve_reviews.py`（REVIEW 批量裁决 CLI，对齐 NashSU 面板的全选+批量按钮；无 `--apply` 只预览）, `review_actions.py`（review 动作路由，移植 NashSU `handleResolve` + `createReviewPageDrafts`；纯函数不落盘，供 process-reviews 判定按钮集/动作含义/建页路由）, `enrich_wikilinks_retroactive.py`, `cross_source_dedup.py`（跨源去重 CLI）, `rebuild_index.py`（index.md 确定性全量重建，不调 LLM，无页数上限；与 Stage 3.3 的 ingest 聚合修复互补） |
