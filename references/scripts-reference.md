...
| Embeddings | **`build_embeddings.py`**（`embed` 全量重建 / `upsert`、`delete` 按页 / `sync [--dry-run]` 对账：只重嵌缺失与已变页、删已消失页 / `compact` / `search` / `stats`；`embed`/`sync`/`delete`/`compact` 需取得 ingest 项目锁，`upsert` 是 Stage 3.7 在 ingest 持锁时调用的子进程）, **`search_wiki.py`** |
