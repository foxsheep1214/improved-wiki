...
def stage_2_per_chunk_generation(
    ...
    """Stage 2 (per-chunk): Generate FILE blocks from each chunk independently.

    Each chunk's analysis is fed to an LLM that generates concept/entity pages
    for the concepts found IN THAT CHUNK.  This avoids the "big synthesis"
    problem where one LLM call has to digest dozens of chunks.

    Chunks are processed in parallel (max 4 concurrent to avoid rate limits),
    then results are deduplicated.  Finally a source page is generated from
    the global digest.
    """
    # chunks: optional list of raw chunk-text strings, reserved for future
    # chunk-text-aware generation. Currently all callers pass []; the chunk
    # analysis dicts (chunk_analyses) carry the structured data the LLM needs.
    chunk_total = len(chunk_analyses)
    print(f"[stage_2] Per-chunk generation: {chunk_total} chunks, "
          f"max {max_chunk_concurrent} concurrent")

    all_file_blocks: list[tuple[str, str]] = []
    all_responses: list[str] = []
    gen_tokens = config.compute_max_tokens(8192)  # per-chunk: smaller tasks

    def _gen_one_chunk(idx: int) -> list[tuple[str, str]]:
        """Generate pages for one chunk."""
    ...
    with ThreadPoolExecutor(max_workers=max_chunk_concurrent) as executor:
        futures = {executor.submit(_gen_one_chunk, i): i for i in range(chunk_total)}
        for future in as_completed(futures):
            blocks = future.result()
            all_file_blocks.extend(blocks)

    # Deduplicate: keep only one block per path (last-write-wins by slug)
    seen_paths: dict[str, int] = {}
    deduped: list[tuple[str, str]] = []
    for path, content in all_file_blocks:
        slug_key = Path(path).stem.lower().replace(" ", "-").replace("_", "-")
        if slug_key in seen_paths:
            # Merge: keep the longer content
            prev_idx = seen_paths[slug_key]
            if len(content) > len(deduped[prev_idx][1]):
                deduped[prev_idx] = (path, content)
        else:
            seen_paths[slug_key] = len(deduped)
...
# Concepts generated ({len(deduped)} pages)
{', '.join(Path(p).stem for p, _ in deduped[:60])}
...
"""
    try:
        ...
        deduped.extend(src_blocks)
    except Exception as e:
        print(f"  [stage_2] Source page generation failed: {e}")

    combined = "\n".join(all_responses)
    concept_blocks = [b for b in deduped if "concepts/" in b[0]]
    entity_blocks = [b for b in deduped if "entities/" in b[0]]
    source_blocks = [b for b in deduped if "sources/" in b[0]]

    print(f"[stage_2] Done — {chunk_total} chunks, {len(combined):,} chars total, "
          f"{len(deduped)} file blocks ({len(concept_blocks)} concepts, "
    ...
    return analysis, combined, deduped
