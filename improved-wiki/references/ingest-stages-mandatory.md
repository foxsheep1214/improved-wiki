...
improved-wiki 流水线 = **19 个 active Stage（含 Phase 0 前置门，跨 5 个 Phase）+ Lint + Graph**。编号与 `ingest.py` 代码一致，**编号即执行顺序**。任何 Stage 都不能跳过。Graph 是独立命令（与 Ingest/Lint 并列，不属于 ingest 管线）。
...
```
0.1 → 0.2 → 1.1 → 1.2 → 1.3 → 2.1 → 2.2 → 2.4 → 2.5 → 2.6 → 2.7 → 2.8 → 2.9
