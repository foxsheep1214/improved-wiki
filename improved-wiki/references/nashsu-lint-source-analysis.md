...
3e. **`suggested_score` is an improved-wiki extension field on broken-link findings** (2026-07-10, user-approved hardening). NashSU never persists a suggestion score — its Fix is human-clicked per item, so the human IS the confidence gate. The headless batch fixer needs an explicit one: `wiki-lint-fix.py` only auto-rewrites when `suggested_score >= 0.9` (exact 1.0 / same-basename 0.96 tiers); contains-tier (0.82) and fuzzy-Levenshtein suggestions route to `REVIEW/suggestion` items instead. A cache without the field (older lint) is treated as below-threshold. Similarly, `wiki-lint.sh`'s delete-orphans stage is now preview + review items only — the batch counterpart of NashSU's per-item Delete button requires an explicit `--delete-orphans --apply`. Both are deliberate non-parity: they restore the human judgment NashSU gets for free from its per-item UI.
