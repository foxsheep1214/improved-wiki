# Stage 2.3 · Query Auto-Generation

> **触发**：Stage 2.1（source/concept/entity generation）完成后自动执行。产物为内存中的 FILE blocks，由 Stage 3.5 统一写盘。。
...
```
你现在已经完成了《{title}》的消化分析（Stage 2.1），已经生成了 source/concept/entity 页面。请基于这些概念列表识别开放问题。。
...
- **no-go**：未输出任何 query block 也未输出 `---QUERIES: 0---` 标记 → Stage 2.3 未完成，重跑
