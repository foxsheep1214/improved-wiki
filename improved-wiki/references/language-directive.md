...
improved-wiki 早就移植了 `scripts/_language.py`（`detect_language` + `build_language_directive` + `IMPROVED_WIKI_OUTPUT_LANGUAGE` 环境变量覆盖），但过去只接入了语义 lint，**没有**接入 ingest 的生成/分析提示词（Stage 2.1/2.2/2.4/2.6/2.7/2.9）。已于 2026-06-29（commit c359232）修复：`build_language_directive(<源文本样本>)` 现在在 `# Role` 之前注入全部 6 个 builder，默认 auto（按源文本检测）。测试覆盖：`test_ingest_language_directive.py`（英文→英文、中文→中文、env 覆盖三种场景）。
