...
|--------|---------------------------|
| `startIngest()` — reads source, starts chat | Calling agent reads source + presents analysis |
| User chats with LLM about the source | You discuss with the calling agent naturally |
| `executeIngestWrites()` — conversation history → FILE blocks | Calling agent uses full conversation context → FILE blocks |
...
The calling agent runs the same Stage 1.1 as auto-ingest:

```bash
# Phase 1 prefetch: text extraction + image extraction + captioning (stops cleanly after Phase 1)
python3 "$SKILL_DIR/scripts/ingest.py" "raw/Book/Book.pdf" --stop-after-stage 0
```
...
The calling agent reads the extracted text and presents a structured **digest** to the user:
...
This is NOT a script output — the calling agent generates this digest directly in the conversation, using the source text it has read. The key is to present **actionable choices**, not just a summary.
...
The calling agent should **confirm understanding** before proceeding:
...
When the user confirms ("生成" / "generate" / "写入" / "proceed"), the calling agent generates wiki pages. The generation prompt **must include** the user's guidance:
...
The calling agent must NOT auto-generate without explicit user confirmation. The whole point of chat ingest is the human-in-the-loop guidance.

## When the calling agent should ask for guidance

After presenting the digest, the calling agent should proactively ask:
...
### Re-ingest with Chat
If a source was previously auto-ingested, chat re-ingest can be used to add missing pages or correct emphasis. The calling agent should:
...
The calling agent should track which sources have been discussed and generated in the current session.
